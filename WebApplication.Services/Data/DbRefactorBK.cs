...
{
    public static class DbRefactorBK
    {
        public static MyContext GetManufacturers()
        {
            var options = new DbContextOptionsBuilder<MyContext>().UseInMemoryDatabase(databaseName: "Test").Options;


            var manufacturerData = new List<Manufacturer>
                    {
            ...
            var vehiclemodelData = new List<VehicleModel>
            {
                new VehicleModel
                {
            ...
            //context.manufacturerRefactor = null;


            foreach (var item in manufacturerData)
              }

               ManufacturerRefactor manufacturerRefactor = new ManufacturerRefactor();
                    manufacturerRefactor.Id = item.Id;
                    manufacturerRefactor.ManufacturerName = item.ManufacturerName;

               List<VehicleModel> vehiclelModelList = new List<VehicleModel>();

                        foreach (var item2 in vehiclemodelData)
                        {
                            if (item2.ManufacturerId == item.Id)
                            {
                            VehicleModel vehicleModel = new VehicleModel();
                            vehicleModel.Id = item2.Id;
                            vehicleModel.ManufacturerId = item2.ManufacturerId;
                            vehicleModel.ModelName = item2.ModelName;
                        

                                vehiclelModelList.Add(vehicleModel);
                            }
                        }

                      manufacturerRefactor.vehicleModel = vehicleModelList;

                        if (!manufacturerRefactor.Equals(null)) { 
                        context.manufacturerRefactor.Add(manufacturerRefactor);
                         return context;
                             context.SaveChanges();
                        }
        ...
        public static MyContext GetModels()
        {
            var options = new DbContextOptionsBuilder<MyContext>().UseInMemoryDatabase(databaseName: "Test").Options;
 
            var manufacturerData2 = new List<Manufacturer>
                    {
                new Manufacturer {Id = "E74EA75C-EF51-40E1-BD83-086805F32060", ManufacturerName = "STARCRAFT"},
                new Manufacturer {Id = "33E32587-A5A8-4CF9-86EE-093197C63776", ManufacturerName = "AUDI"},
                new Manufacturer {Id = "2D2158D4-96DA-4EBC-A62C-0C41EE98F8E3", ManufacturerName = "LAMBORGHINI"},
            ...
            var vehiclemodelData2 = new List<VehicleModel>
            {
                new VehicleModel
                {
                    Id = "066E509D-4C23-4C70-B7C5-00116F047889", ModelName = "マークIIワゴン",
            ...
            var context = new MyContext(options);
           // context.vehicleModelRefactor = null;
           
                foreach (var item in vehiclemodelData2)
                {
                    VehicleModelRefactor vehicleModelRefactor = new VehicleModelRefactor();
                    vehicleModelRefactor.Id = item.Id;
                    vehicleModelRefactor.ModelName = item.ModelName;
                    vehicleModelRefactor.ManufacturerId = item.ManufacturerId;

                    Manufacturer manufacturer = new Manufacturer();

                    var query = (from m in manufacturerData2
                                 where m.Id == item.ManufacturerId
                                 select new { m.Id, m.ManufacturerName }).ToList();

                    foreach (var item2 in query)
                    {

                        manufacturer.Id = item2.Id;
                        manufacturer.ManufacturerName = item2.ManufacturerName;

                    }


                    vehicleModelRefactor.manufacturer = manufacturer;

                if (!vehicleModelRefactor.Equals(null))
                    context.vehicleModelRefactor.Add(vehicleModelRefactor);
                    context.SaveChanges();
                }
